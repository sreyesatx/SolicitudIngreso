...
namespace SistemaSolicitudIngreso.Administracion
{
    public partial class Administracion : System.Web.UI.Page
    {
        ...
        {
            CheckBoxEnvioCorreo.Checked = true;
            int cont = 0;
            ...
            {
                ScriptManager.RegisterStartupScript(Page, this.GetType(), "alert", string.Format("swal('¡Debe seleccionar Solicitud Ingreso!');", Title), true);
            }
        }
        ...
        protected void btnEnviar_1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ddlEstado_1.SelectedIndex == 0)
                {
                    ScriptManager.RegisterStartupScript(Page, this.GetType(), "alert", string.Format("swal('¡Seleccione estado!');", Title), true);
                    return;
                }
                /*
                if (ddlMensajeCorreoMasivo.Enabled && ddlEstado_1.SelectedValue.ToString() == "6" && ddlMensajeCorreoMasivo.SelectedIndex == 0)
                {
                    ScriptManager.RegisterStartupScript(Page, this.GetType(), "alert", string.Format("swal('¡Debe seleccionar Opción Mensaje Correo!');", Title), true);
                    return;
                }
                */
                ...
                if (CheckBoxEnvioCorreo.Checked)
                {
                    ...
                    if (datos != null)
                    {
                        if (datos.Count != 0)
                        {
                            ...
                            */
                            /////////////////////////////////////////
                            if (ddlEstado_1.SelectedItem.Text.ToUpper() == "AUTORIZADO")
                            {
                                if (ddlMensajeCorreoMasivo.SelectedIndex > 0)
                                {
                                    mensaje = "Su solicitud de acceso a las instalaciones ha sido " + ddlEstado_1.SelectedItem.Text.ToUpper() + ". " + ddlMensajeCorreoMasivo.SelectedValue.ToString();
                                }
                                else
                                {
                            ...
                            if (ddlEstado_1.SelectedItem.Text == "Autorizado")
                            {
                                ...
                                ScriptManager.RegisterStartupScript(Page, this.GetType(), "alert", string.Format("swal('¡Gestión realizada!');", Title), true);
                            }
        ...
        protected void Enviar(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                /*
                if (ddlMensajeCorreo.Enabled && ddlEstado.SelectedValue.ToString() == "6" && ddlMensajeCorreo.SelectedIndex == 0)
                {
                    ScriptManager.RegisterStartupScript(Page, this.GetType(), "alert", string.Format("swal('¡Debe seleccionar Opción Mensaje Correo!');", Title), true);
                    return;
                }
                ...
                if (CheckBox1.Checked)
                {
                    ...
                    if (datos != null)
                    {
                        if (datos.Count != 0)
                        {
                            ...
                            /////////////////////////////////////////
                            if (ddlEstado.SelectedItem.Text.ToUpper() == "AUTORIZADO")
                            {
                                if (ddlMensajeCorreo.SelectedIndex > 0)
                                {
                                    mensaje = "Su solicitud de acceso a las instalaciones ha sido " + ddlEstado.SelectedItem.Text.ToUpper() + ". " + ddlMensajeCorreo.SelectedValue.ToString();
                                }
                                else
                                {
